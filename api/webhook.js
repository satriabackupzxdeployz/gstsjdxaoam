...
const API_KEY = process.env.THERESAV_APIKEY || "ShikimoriChan";
...
async function sendReaction(chat_id, message_id, emoji) {
  try {
    ...
    // Reaction API mungkin tidak didukung semua versi, abaikan error
  }
}
...
async function handleAmPremium(msg, email) {
  ...
  try {
    ...
    await sendMessage(
      chat_id,
      `❌ *Gagal Mengirim Premium:* ${e.response?.data?.message || e.message}`
    );
  }
}
...
async function handleAmVerify(msg, args) {
  ...
  if (!email || !link) {
    return sendMessage(
      chat_id,
      `⚠️ *Input Kurang Lengkap kak!*\n\nPastikan memasukkan email dan link dengan pemisah tanda pipa (\\|).\n\n*Contoh:* /amverify email@gmail.com | https://link...`
    );
  }
...
async function processUpdate(update) {
  ...
  // hapus @botname jika ada
  const cmd = rawCmd.split("@")[0].toLowerCase();
  const args = rest.join(" ");

  if (cmd === "/start" || cmd === "/help") {
    await handleStart(msg);
  } else if (cmd === "/ampremium" || cmd === "/sendam" || cmd === "/alightpremium" || cmd === "/alightmotion") {
    if (!args) {
      await sendMessage(
        msg.chat.id,
        `📧 *Format Salah kak!*\n\nKetik: /ampremium \\<email\\_kamu\\>\n\nContoh:\n/ampremium emailkamu@gmail.com`,
        "MarkdownV2"
      );
    } else {
      await handleAmPremium(msg, args);
    }
  } else if (cmd === "/amverify" || cmd === "/alightverify" || cmd === "/viam" || cmd === "/verifyam") {
